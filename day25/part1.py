...
def solve():
    ...
    betweenness = networkx.edge_betweenness_centrality(graph)
    crucial_edges = sorted(betweenness, key=betweenness.get)[-3:]
    graph.remove_edges_from(crucial_edges)
    group_size = [len(c) for c in networkx.connected_components(graph)]
...
try:
    with open(file_path, "r") as file:
        input = file.read()
        print("Product of groups' sizes:", solve())
