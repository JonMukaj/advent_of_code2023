...
def solve(hails, N):
    variables = sp.symbols("x y z vx vy vz " + " ".join(f"t_{i}" for i in range(N)))
    equations = []
    for i, (x0, y0, z0, vx0, vy0, vz0) in enumerate(hails[:N]):
        ...
        equations += [sp.Eq(*map(sp.S, formula.split("=", 1))) for formula in formulas]
    roots = sp.solve(equations, variables)[0]
    return sum(roots[:3])
...
try:
    with open(file_path, "r") as file:
        ...
        for line in file:
            ...
            hails.append((*x, *v))
        print(f"Sum of coordinates: {solve(hails, 5)}")
